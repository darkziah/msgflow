...
The ingress Worker (Hono) and the Conversation Durable Object are deployed as one Worker: the Worker serves the Meta webhook, Gmail Pub/Sub push, and the authenticated app API, and also exports the `ConversationDO` class, reached via `env.CONVERSATION_DO.idFromName(conversationId)`.
